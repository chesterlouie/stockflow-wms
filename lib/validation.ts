...
export const barcodeSchema = z.object({
  barcodeMode: z.enum(["auto", "manual"]),
  barcodeValue: z.string().trim().max(120).optional(),
  barcodeFormat: z.enum(["code128","ean13","upca","qr","gs1"]),
  uom: z.string().trim().min(1).max(20),
  quantityInBase: z.coerce.number().positive().max(999999999),
}).refine((value) => value.barcodeMode === "auto" || Boolean(value.barcodeValue), { message: "A barcode is required in manual mode", path: ["barcodeValue"] });
...
const optionalLocationId=z.preprocess(value=>value===''?undefined:value,z.string().uuid().optional());
export const inspectionSchema=z.object({lineId:z.string().uuid(),receivingLocationId:z.string().uuid(),holdLocationId:optionalLocationId,damagedLocationId:optionalLocationId,putawayLocationId:z.string().uuid(),acceptedQuantity:z.coerce.number().min(0),heldQuantity:z.coerce.number().min(0),damagedQuantity:z.coerce.number().min(0),lotNumber:z.string().trim().max(100).optional(),expiryDate:z.string().optional(),serialNumbers:z.string().trim().max(20000).optional(),receiptBarcode:z.string().trim().max(120).optional()}).refine(v=>v.acceptedQuantity+v.heldQuantity+v.damagedQuantity>0);

export const salesOrderSchema=z.object({warehouseId:z.string().uuid(),storeId:z.string().uuid().optional().or(z.literal('')),orderNo:z.string().trim().min(1).max(50),customer:z.string().trim().max(150).optional(),requestedShipDate:z.string().optional(),priority:z.enum(['low','normal','high','urgent']),itemId:z.string().uuid(),quantity:z.coerce.number().positive().max(999999999),uom:z.string().trim().min(1).max(20)}).refine(v=>Boolean(v.storeId||v.customer?.trim()),{message:'Select a store or enter a customer',path:['storeId']});
