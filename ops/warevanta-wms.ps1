...
function Show-Status {
  ...
  $application=if(Get-PortProcessId $appPort){'running'}else{'stopped'}
  ...
  if($database -eq 'running' -and $application -eq 'running' -and $https -eq 'running'){
    try{$ready=Invoke-RestMethod -Uri "http://127.0.0.1:$appPort/api/ready" -TimeoutSec 10;Write-Output "Health:      $($ready.status) (database $($ready.database), migration $($ready.migration))"}catch{Write-Output "Health:      failed - $($_.Exception.Message)"}
  }
}
function Start-WarevantaWms {
  ...
  if(-not(Get-PortProcessId $appPort)){
    Write-Output 'Starting Warevanta application...'
    Start-CleanProcess $node "--env-file=.env.production node_modules/vinext/dist/cli.js start --hostname 127.0.0.1 --port $appPort"
    Wait-ForPort $appPort
  ...
  $ready=Invoke-RestMethod -Uri "http://127.0.0.1:$appPort/api/ready" -TimeoutSec 15
...
switch($Action){
  'start'{Start-WarevantaWms}
  'stop'{Stop-WarevantaWms}
  'restart'{Stop-WarevantaWms;Start-WarevantaWms}
  'status'{Show-Status}
}
