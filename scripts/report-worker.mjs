...
async function processCarrierTenderCapacity(){if(!(await client.query(`SELECT pg_try_advisory_lock(9042089) ok`)).rows[0].ok)return;try{await client.query('BEGIN');const due=(await client.query(`SELECT t.id,t.company_id,t.assignment_id,t.attempt_number,t.status trigger_status,a.order_id,o.warehouse_id,o.store_id,o.order_no,coalesce(a.promised_dispatch_at::date,current_date) service_date,(SELECT coalesce(sum(quantity),0) FROM sales_order_lines WHERE order_id=o.id) units,coalesce(cfg.max_attempts,3) max_attempts,coalesce(cfg.escalation_minutes,30) escalation_minutes,coalesce(cfg.auto_retender_on_expiry,true) auto_expiry,coalesce(cfg.auto_retender_on_rejection,true) auto_rejection FROM carrier_tender_attempts t JOIN order_carrier_assignments a ON a.id=t.assignment_id JOIN sales_orders o ON o.id=a.order_id LEFT JOIN carrier_tender_automation_configs cfg ON cfg.company_id=t.company_id AND cfg.enabled WHERE t.terminal_reason IS NULL AND((t.status='submitted' AND t.response_due_at<now()) OR t.status='rejected') AND t.attempt_number=(SELECT max(x.attempt_number) FROM carrier_tender_attempts x WHERE x.assignment_id=t.assignment_id) FOR UPDATE OF t,a SKIP LOCKED`)).rows;for(const x of due){if(x.trigger_status==='submitted'){await client.query(`UPDATE carrier_tender_attempts SET status='expired',responded_at=now(),response_note='Response SLA expired automatically.' WHERE id=$1`,[x.id]);await client.query(`UPDATE order_carrier_assignments SET status='rejected',updated_at=now() WHERE id=$1`,[x.assignment_id]);await client.query(`INSERT INTO carrier_assignment_events(company_id,assignment_id,event_type,note) VALUES($1,$2,'rejected','Carrier response SLA expired; capacity released.')`,[x.company_id,x.assignment_id])}await client.query(`UPDATE carrier_capacity_reservations SET status='released',released_at=now() WHERE assignment_id=$1 AND status='reserved'`,[x.assignment_id]);const enabled=x.trigger_status==='submitted'?x.auto_expiry:x.auto_rejection;if(enabled&&Number(x.attempt_number)<Number(x.max_attempts)){const used=(await client.query(`SELECT carrier_service_id FROM carrier_tender_attempts WHERE assignment_id=$1`,[x.assignment_id])).rows.map(y=>y.carrier_service_id),candidate=(await client.query(`SELECT r.*,l.id limit_id,l.response_sla_minutes FROM carrier_award_recommendations r JOIN carrier_capacity_limits l ON l.company_id=r.company_id AND l.carrier_service_id=r.carrier_service_id AND l.warehouse_id=r.warehouse_id AND $3 BETWEEN l.effective_from AND l.effective_to AND l.active LEFT JOIN LATERAL(SELECT count(*) shipments,coalesce(sum(shipment_units),0) units FROM carrier_capacity_reservations WHERE capacity_limit_id=l.id AND service_date=$3 AND status IN('reserved','consumed'))u ON true WHERE r.company_id=$1 AND r.order_id=$2 AND NOT(r.carrier_service_id=ANY($4::uuid[])) AND coalesce(u.shipments,0)<l.max_shipments AND coalesce(u.units,0)+$5<=l.max_units ORDER BY r.recommendation_rank LIMIT 1 FOR UPDATE OF l`,[x.company_id,x.order_id,x.service_date,used,x.units])).rows[0];if(candidate){const reservation=(await client.query(`INSERT INTO carrier_capacity_reservations(company_id,assignment_id,capacity_limit_id,service_date,shipment_units) VALUES($1,$2,$3,$4,$5) ON CONFLICT(company_id,assignment_id) DO UPDATE SET capacity_limit_id=excluded.capacity_limit_id,service_date=excluded.service_date,shipment_units=excluded.shipment_units,status='reserved',reserved_at=now(),released_at=null RETURNING id`,[x.company_id,x.assignment_id,candidate.limit_id,x.service_date,x.units])).rows[0];await client.query(`UPDATE carrier_tender_attempts SET terminal_reason='automatic_fallback_started' WHERE id=$1`,[x.id]);const tender=(await client.query(`INSERT INTO carrier_tender_attempts(company_id,assignment_id,attempt_number,carrier_service_id,capacity_reservation_id,response_due_at,initiation_source,trigger_attempt_id) VALUES($1,$2,$3,$4,$5,now()+($6||' minutes')::interval,'automatic',$7) RETURNING id`,[x.company_id,x.assignment_id,Number(x.attempt_number)+1,candidate.carrier_service_id,reservation.id,candidate.response_sla_minutes,x.id])).rows[0];await client.query(`UPDATE order_carrier_assignments SET carrier_service_id=$2,selected_rank=$3,award_score=$4,status='tendered',updated_at=now() WHERE id=$1`,[x.assignment_id,candidate.carrier_service_id,candidate.recommendation_rank,candidate.award_score]);await client.query(`INSERT INTO carrier_assignment_events(company_id,assignment_id,event_type,note) VALUES($1,$2,'tendered',$3)`,[x.company_id,x.assignment_id,`Automatically retendered as attempt ${Number(x.attempt_number)+1}.`]);await notifyTenderManagers(x,candidate.carrier_service_id,`carrier-tender:${tender.id}:fallback`,'Carrier tender automatically reassigned',`${x.order_no} moved to ranked carrier #${candidate.recommendation_rank}.`,'high');continue}}await client.query(`UPDATE carrier_tender_attempts SET terminal_reason=$2 WHERE id=$1`,[x.id],).catch(()=>{});await client.query(`UPDATE carrier_tender_attempts SET terminal_reason='fallback_exhausted' WHERE id=$1`,[x.id]);await client.query(`INSERT INTO operational_exceptions(company_id,domain,entity_type,entity_id,order_id,warehouse_id,store_id,category,severity,summary,recommended_action,sla_due_at) VALUES($1,'dispatch','carrier_assignment',$2,$3,$4,$5,'carrier_tender_exhausted','critical',$6,'Review capacity or manually negotiate a carrier.',now()+($7||' minutes')::interval) ON CONFLICT(company_id,domain,entity_type,entity_id,category) WHERE status<>'resolved' DO UPDATE SET severity='critical',summary=excluded.summary,updated_at=now()`,[x.company_id,x.assignment_id,x.order_id,x.warehouse_id,x.store_id,`${x.order_no} exhausted all eligible carrier tender options.`,x.escalation_minutes]);await notifyTenderManagers(x,null,`carrier-tender:${x.assignment_id}:exhausted`,'Carrier tender options exhausted',`${x.order_no} has no eligible carrier capacity remaining.`,'critical')}await client.query('COMMIT')}catch(e){await client.query('ROLLBACK');throw e}finally{await client.query(`SELECT pg_advisory_unlock(9042089)`)}}
async function notifyTenderManagers(x,carrierServiceId,eventKey,title,message,priority){await client.query(`INSERT INTO carrier_sla_notifications(company_id,user_id,carrier_service_id,warehouse_id,event_key,title,message,link_path,entity_type,entity_id,priority) SELECT $1,m.user_id,$2,$3,$4,$5,$6,'/app/carrier-tenders','carrier_assignment',$7,$8 FROM company_members m WHERE m.company_id=$1 AND m.role IN('owner','admin','manager') AND(m.role='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments u WHERE u.company_id=$1 AND u.user_id=m.user_id AND u.warehouse_id=$3)) ON CONFLICT DO NOTHING`,[x.company_id,carrierServiceId,x.warehouse_id,eventKey,title,message,x.assignment_id,priority])}
