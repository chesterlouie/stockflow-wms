...
if (!deliveryEnabled) {
  ...
  const safeTimer = setInterval(
    () =>
      Promise.all([createAlerts(), processApprovals(),processCountSchedules(),processVkitBackorders(),processOperationalExceptions(),processDeliveryExceptions(),processCarrierTenderExceptions(),processCarrierIntegrationJobs()]).catch(console.error),
    60000,
  );
