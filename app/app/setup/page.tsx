...
type Location = { id: string; warehouse_id:string; warehouse_name: string; code: string; type: string };
...
export default async function Setup({ searchParams }: { searchParams: Promise<{ created?: string; warehouseCreated?: string; error?: string }> }) {
  ...
  const locations = session ? await tenantRows<Location>(session.companyId, `SELECT l.id,l.warehouse_id,w.name AS warehouse_name,l.code,l.type FROM locations l JOIN warehouses w ON w.company_id=l.company_id AND w.id=l.warehouse_id WHERE l.company_id=$1 ORDER BY w.name,l.code`, [session.companyId]) : [];
  ...
  return <div className="app-content">
    ...
    <section className="panel">
      ...
      <div className="admin-table-wrap"><table className="data-table"><thead><tr><th>Code</th><th>Warehouse</th><th>Timezone</th><th>Status</th><th>Labels</th></tr></thead><tbody>{warehouses.map(warehouse => <tr key={warehouse.id}><td><strong>{warehouse.code}</strong></td><td>{warehouse.name}</td><td>{warehouse.timezone}</td><td><span className="badge">{warehouse.active ? "Active" : "Inactive"}</span></td><td><Link className="table-link" href={`/app/setup/locations/labels?warehouseId=${warehouse.id}`}>Print all locations</Link></td></tr>)}</tbody></table></div>
    </section>
    <div className="form-layout">
      <section className="panel"><h2>Locations</h2><div className="admin-table-wrap"><table className="data-table"><thead><tr><th>Warehouse</th><th>Location</th><th>Type</th><th>Status</th><th>Label</th></tr></thead><tbody>{locations.length ? locations.map(location => <tr key={location.id}><td>{location.warehouse_name}</td><td><strong>{location.code}</strong></td><td>{location.type}</td><td><span className="badge">Active</span></td><td><Link className="table-link" href={`/app/setup/locations/${location.id}/label`}>Print label</Link></td></tr>) : <tr><td colSpan={5} className="empty-cell">Create a receiving or storage location to begin.</td></tr>}</tbody></table></div></section>
