  ...
  return <div className="app-content">
    ...
    <div className="form-layout">
      ...
      <aside className="panel">
        <h2>Add warehouse</h2>
        {canManageWarehouses ? <form className="form-stack compact-form" method="post" action="/api/warehouses"><div className="field"><label htmlFor="warehouseCode">Warehouse code</label><input id="warehouseCode" name="code" placeholder="e.g. CEB" required /></div><div className="field"><label htmlFor="warehouseName">Warehouse name</label><input id="warehouseName" name="name" placeholder="e.g. Cebu Distribution Center" required /></div><div className="field"><label htmlFor="timezone">Timezone</label><input id="timezone" name="timezone" defaultValue="Asia/Manila" required /></div><button className="button button-primary" type="submit" disabled={atLimit}>Create warehouse</button>{atLimit && <p className="form-note">Upgrade the subscription to add another warehouse. Existing warehouses and inventory remain available.</p>}</form> : <p className="form-note">Only company owners and administrators can add warehouses.</p>}
