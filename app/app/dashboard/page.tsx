...
export default async function Dashboard() {
  ...
  const m = (s
    ? await tenantRows<Metrics>(
        s.companyId,
        `SELECT (SELECT coalesce(sum(b.quantity),0)::text FROM inventory_balances b WHERE b.company_id=$1 AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=b.company_id AND ua.user_id=$3 AND ua.warehouse_id=b.warehouse_id))) AS units,(SELECT count(DISTINCT b.item_id)::text FROM inventory_balances b WHERE b.company_id=$1 AND b.quantity>0 AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=b.company_id AND ua.user_id=$3 AND ua.warehouse_id=b.warehouse_id))) AS skus,(SELECT count(*)::text FROM sales_orders o WHERE o.company_id=$1 AND o.status<>'dispatched' AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=o.company_id AND ua.user_id=$3 AND ua.warehouse_id=o.warehouse_id))) AS orders,(SELECT count(*)::text FROM inventory_counts c WHERE c.company_id=$1 AND c.status IN('open','counted') AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=c.company_id AND ua.user_id=$3 AND ua.warehouse_id=c.warehouse_id))) AS open_counts,(SELECT count(*)::text FROM inventory_balances b WHERE b.company_id=$1 AND b.expiry_date<=current_date+30 AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=b.company_id AND ua.user_id=$3 AND ua.warehouse_id=b.warehouse_id))) AS expiring,(SELECT coalesce(round(100-100*sum(abs(cl.counted_quantity-cl.system_quantity))/nullif(sum(abs(cl.system_quantity)),0),1),100)::text FROM inventory_count_lines cl JOIN inventory_counts c ON c.id=cl.count_id WHERE cl.company_id=$1 AND cl.status='approved' AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=c.company_id AND ua.user_id=$3 AND ua.warehouse_id=c.warehouse_id))) AS accuracy`,
  ...
  const o = (s
    ? await tenantRows<Ops>(
        s.companyId,
        `SELECT coalesce(sum(m.quantity) FILTER(WHERE m.movement_type IN('receipt','inspection_accepted','inspection_hold','inspection_damaged')),0)::text AS received,coalesce(sum(m.quantity) FILTER(WHERE m.movement_type='pick_in'),0)::text AS picked,coalesce(-sum(m.quantity) FILTER(WHERE m.movement_type='shipment_issue'),0)::text AS shipped,coalesce(sum(abs(m.quantity)) FILTER(WHERE m.movement_type IN('adjustment','count_adjustment')),0)::text AS adjusted FROM inventory_ledger m WHERE m.company_id=$1 AND m.occurred_at>=current_date AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=m.company_id AND ua.user_id=$3 AND ua.warehouse_id=m.warehouse_id))`,
        [s.companyId,s.role,s.userId],
      )
  ...
  const activities = s
    ? await tenantRows<Activity>(
        s.companyId,
        `SELECT m.movement_type,m.quantity::text,m.uom,i.sku,l.code AS location,m.reference_id,m.occurred_at::text FROM inventory_ledger m JOIN items i ON i.id=m.item_id JOIN locations l ON l.id=m.location_id WHERE m.company_id=$1 AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=m.company_id AND ua.user_id=$3 AND ua.warehouse_id=m.warehouse_id)) ORDER BY m.occurred_at DESC LIMIT 4`,
        [s.companyId,s.role,s.userId],
      )
    : [];
  const controls=(s?await tenantRows<Controls>(s.companyId,`SELECT (SELECT count(*)::text FROM approval_requests WHERE company_id=$1 AND status='pending') pending_approvals,(SELECT count(*)::text FROM approval_requests q JOIN approval_rules r ON r.id=q.rule_id WHERE q.company_id=$1 AND q.status='pending' AND q.requested_at+(r.escalation_hours||' hours')::interval<now()) overdue_approvals,(SELECT count(*)::text FROM fulfillment_exceptions e JOIN sales_orders o ON o.id=e.order_id WHERE e.company_id=$1 AND e.status='open' AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=o.company_id AND ua.user_id=$3 AND ua.warehouse_id=o.warehouse_id))) exceptions,(SELECT count(*)::text FROM sales_orders o WHERE o.company_id=$1 AND o.status='packed' AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=o.company_id AND ua.user_id=$3 AND ua.warehouse_id=o.warehouse_id))) ready_dispatch,(SELECT count(*)::text FROM inventory_counts c WHERE c.company_id=$1 AND c.status='counted' AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=c.company_id AND ua.user_id=$3 AND ua.warehouse_id=c.warehouse_id))) counted,(SELECT count(*)::text FROM inbound_receipts r WHERE r.company_id=$1 AND r.status IN('expected','partial','inspected','putaway') AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments ua WHERE ua.company_id=r.company_id AND ua.user_id=$3 AND ua.warehouse_id=r.warehouse_id))) inbound`,[s.companyId,s.role,s.userId]):[])[0]||{pending_approvals:'0',overdue_approvals:'0',exceptions:'0',ready_dispatch:'0',counted:'0',inbound:'0'};
  const backorderControls=(s&&isSupervisor?await tenantRows<{open:string;overdue:string;critical:string}>(s.companyId,`SELECT count(*) FILTER(WHERE o.backorder_status IN('waiting','ready'))::text open,count(*) FILTER(WHERE b.attention_status='overdue')::text overdue,count(*) FILTER(WHERE b.attention_status='critical')::text critical FROM vkit_backorder_summary b JOIN sales_orders o ON o.company_id=b.company_id AND o.id=b.id WHERE b.company_id=$1 AND ($2='owner' OR EXISTS(SELECT 1 FROM user_warehouse_assignments a WHERE a.company_id=b.company_id AND a.user_id=$3 AND a.warehouse_id=b.warehouse_id))`,[s.companyId,s.role,s.userId]):[])[0]||{open:'0',overdue:'0',critical:'0'};
  const attention=Number(m.expiring)+Number(controls.overdue_approvals)+Number(controls.exceptions)+Number(controls.counted)+Number(backorderControls.overdue)+Number(backorderControls.critical);
  const max = Math.max(
    Number(o.received),
    Number(o.picked),
    Number(o.shipped),
    ...
    <div className="app-content dashboard-page">
      ...
      <section className="attention-banner">
        ...
        <div>
          ...
          <small>
            {controls.overdue_approvals} overdue approvals, {backorderControls.overdue} overdue and {backorderControls.critical} critical VKIT backorders, {controls.exceptions} fulfillment exceptions, {controls.counted} counts awaiting approval, and {m.expiring} expiring lots.
          </small>
        </div>
        <Link href={isSupervisor?(Number(backorderControls.critical)||Number(backorderControls.overdue)?'/app/orders/backorders':Number(controls.overdue_approvals)?'/app/approvals':Number(controls.exceptions)?'/app/exceptions':'/app/counts'):isOperator?'/app/receiving':'/app/reports?type=expiry'}>Review items →</Link>
      </section>
