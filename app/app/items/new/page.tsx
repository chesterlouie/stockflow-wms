  ...
  return <div className="app-content">
    ...
    <form className="form-layout" method="post" action="/api/items">
      <section className="panel form-section">
        ...
        <div className="form-row"><div className="field"><label htmlFor="category">Category</label><select id="category" name="category"><option value="">Uncategorized</option>{categories.map((category) => <option key={category.name} value={category.name}>{category.name}</option>)}</select><small className="form-note">Maintain choices under Master data → Categories.</small></div><div className="field"><label htmlFor="itemType">Item type</label><select id="itemType" name="itemType"><option value="standard">Standard item</option><option value="spare_part">Spare part</option><option value="virtual_kit">Virtual kit (components allocated)</option><option value="stocked_kit">Stocked kit (held as inventory)</option></select></div></div>
        <div className="field"><label htmlFor="uom">Base unit</label><select id="uom" name="uom"><option value="EA">EA — Each</option><option value="CASE">CASE — Case</option><option value="KG">KG — Kilogram</option><option value="BAG">BAG — Bag</option></select></div>
        <h2>Barcode ID</h2>
