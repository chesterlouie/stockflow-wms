...
export default function ModuleFive(){return <section className="panel" id="module-5">
  ...
  <div className="knowledge-note"><strong>Two different scans:</strong> scan the receiving-label QR code to find the task, then scan the product&apos;s registered item barcode in Confirm item barcode or SKU. The exact SKU may be entered as a controlled fallback. Scanning the receiving label in the item field will be rejected.</div>
  ...
  <p>Scan a receipt label, PO, SKU, or barcode to find the task. Confirm the item with a registered EA/CASE barcode or the exact SKU before posting quantities and tracking data. A value belonging to another item is rejected. Successful posting returns to the mobile queue; validation failures remain on the task and explain whether the barcode, quantity, lot, expiry, shelf life, location, or receipt state is blocking the transaction.</p>
  <div className="knowledge-note"><strong>Company and environment matter:</strong> a receipt label only opens a task inside the company workspace that created it. A completed receipt or a label from another company will not appear as open work. Each operator must have an accepted, verified account in that same company; signing in from another device does not require manager approval.</div>
