...
export default async function MobileReceiving({searchParams}:{searchParams:Promise<{barcode?:string;received?:string}>}){
  ...
  return <div className="app-content">
    ...
    <section className="panel inventory-history"><h2>{barcode?'Matching open tasks':'Next open tasks'}</h2>{tasks.length?tasks.map(task=><Link key={task.id} href={`/app/receiving/mobile/${task.id}`} className="pick-card"><div><strong>{task.receipt_no} · {task.sku}</strong><small>{task.supplier} · PO {task.external_reference||'—'}</small><small>{task.description}</small></div><span className="badge">{task.remaining} {task.uom} remaining</span></Link>):<div className="empty-cell"><p>No open receiving task matches that scan in your assigned warehouses.</p>{barcode&&<small>Confirm that the receipt is open and assigned to a warehouse in your Users and access profile.</small>}</div>}</section>
  </div>;
}
