...
export default async function BillingPage({ searchParams }: { searchParams: Promise<{ consultation?: string; pending?: string; cancelled?: string; error?: string }> }) {
  ...
  return <div className="app-content billing-page">
    ...
    <section className="pricing-heading"><div><h2>Choose the right plan</h2><p>Starter is priced per user. Growth and Business are fixed monthly bundles. Annual billing includes two months at no additional cost.</p></div><div className="payment-safety">Card and wallet details are handled securely by PayMongo</div></section>
    <section className="plan-grid">{plans.map((plan) => { const isCurrent = company?.subscription_plan === plan.id && company?.subscription_status === "active"; return <article key={plan.id} className={`plan-card ${"featured" in plan && plan.featured ? "featured" : ""} ${isCurrent ? "current" : ""}`}>
      ...
      <div className="plan-price">{plan.price === null ? <strong>Let&apos;s talk</strong> : <><small>₱</small><strong>{plan.price.toLocaleString()}</strong><span>{plan.perUser ? "/ user / month" : "/ month"}</span></>}</div>
      ...
      {canManage ? <form method="post" action="/api/billing/plan"><input type="hidden" name="plan" value={plan.id}/><label className="billing-cycle-label">Billing cycle<select name="cycle" defaultValue={company?.billing_cycle || "monthly"}><option value="monthly">Monthly{planAmount(plan.id, "monthly") ? ` · ₱${planAmount(plan.id, "monthly")?.toLocaleString()}${plan.perUser ? " per user" : ""}` : ""}</option><option value="annual">Annual{planAmount(plan.id, "annual") ? ` · ₱${planAmount(plan.id, "annual")?.toLocaleString()}${plan.perUser ? " per user" : ""}` : ""}</option></select></label><button className={`button ${"featured" in plan && plan.featured ? "button-primary" : "button-secondary"}`} type="submit" disabled={isCurrent}>{isCurrent ? "Current plan" : plan.id === "enterprise" ? "Request consultation" : "Continue to PayMongo"}</button></form> : <p className="form-note">Only company owners and administrators can change the plan.</p>}
    </article>})}</section>
