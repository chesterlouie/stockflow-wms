...
type StoreKit={item_id:string;sku:string;description:string;base_uom:string;available_kits:string;components:Array<{item_id:string;sku:string;description:string;quantity:string;uom:string}>};
const escapeHtml=(value:string)=>value.replaceAll('&','&amp;').replaceAll('<','&lt;').replaceAll('>','&gt;').replaceAll('"','&quot;').replaceAll("'",'&#39;');

function installVkitReturnForm(kits:StoreKit[]){const host=document.querySelector<HTMLElement>('#transactions');if(!host||host.querySelector('[data-vkit-return]')||!kits.length)return;const form=document.createElement('form');form.dataset.vkitReturn='true';form.className='form-stack panel';form.method='post';form.action='/api/store-transactions/vkit-return';const title=document.createElement('h3');title.textContent='Return a virtual kit';const note=document.createElement('p');note.className='form-note';note.textContent='Warevanta expands the parent kit into required component lines. Scan every component before submitting for manager approval.';const kitLabel=document.createElement('label');kitLabel.textContent='Virtual kit';const select=document.createElement('select');select.name='kitItemId';select.required=true;for(const kit of kits){const option=document.createElement('option');option.value=kit.item_id;option.textContent=`${kit.sku} — ${kit.description} (${kit.available_kits} complete available)`;select.append(option)}kitLabel.append(select);const quantityLabel=document.createElement('label');quantityLabel.textContent='Complete kits to return';const quantity=document.createElement('input');quantity.name='quantity';quantity.type='number';quantity.min='0.000001';quantity.step='any';quantity.required=true;quantityLabel.append(quantity);const locationLabel=document.createElement('label');locationLabel.textContent='Source locator';const location=document.createElement('input');location.name='locationCode';location.required=true;location.dataset.scanField='true';location.placeholder='Scan store locator';locationLabel.append(location);const components=document.createElement('div');const refresh=()=>{const kit=kits.find(x=>x.item_id===select.value);components.replaceChildren();for(const component of kit?.components||[]){const label=document.createElement('label');label.textContent=`${component.sku} — ${component.description} · ${component.quantity} ${component.uom} per kit`;const input=document.createElement('input');input.name=`component_${component.item_id}_barcode`;input.required=true;input.dataset.scanField='true';input.placeholder='Scan component barcode or exact SKU';label.append(input);components.append(label)}};select.onchange=refresh;refresh();const reasonLabel=document.createElement('label');reasonLabel.textContent='Reason code';const reason=document.createElement('input');reason.name='reasonCode';reason.required=true;reason.placeholder='RETURN_TO_WAREHOUSE';reasonLabel.append(reason);const notes=document.createElement('textarea');notes.name='notes';notes.placeholder='Return condition and supporting details';const submit=document.createElement('button');submit.className='button button-primary';submit.textContent='Submit VKIT return for approval';form.append(title,note,kitLabel,quantityLabel,locationLabel,components,reasonLabel,notes,submit);host.prepend(form)}
  ...
  useEffect(() => {
    ...
    const enhance = () => {
      ...
      if(availability&&!availability.querySelector('[data-store-kit-availability]')){
        ...
        fetch('/api/store-kit-availability',{cache:'no-store'}).then(response=>response.ok?response.json():Promise.reject()).then(({kits}:{kits:StoreKit[]})=>{const body=kits.length?kits.map(kit=>`<tr><td><strong>${escapeHtml(kit.sku)}</strong><small>${escapeHtml(kit.description)}</small></td><td><strong>${escapeHtml(kit.available_kits)} ${escapeHtml(kit.base_uom)}</strong></td></tr>`).join(''):'<tr><td colspan="2" class="empty-cell">No virtual kits are configured.</td></tr>';panel.innerHTML=`<strong>Virtual kit availability</strong><p>Buildable now from approved component stock. Component balances remain the inventory of record.</p><div class="store-table-wrap"><table class="data-table"><thead><tr><th>Virtual kit</th><th>Complete kits</th></tr></thead><tbody>${body}</tbody></table></div>`;installVkitReturnForm(kits)}).catch(()=>{panel.innerHTML='<strong>Virtual kit availability</strong><p>Availability could not be loaded. Refresh or try again.</p>'});
      }
      document
