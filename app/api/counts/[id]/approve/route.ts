...
export async function POST(
  ...
  try {
    ...
    await withTenant(s.companyId, async (c) => {
      const count = (
        await c.query(
          `SELECT * FROM inventory_counts c WHERE company_id=$1 AND id=$2 AND status='counted' AND NOT EXISTS(SELECT 1 FROM inventory_counts r WHERE r.parent_count_id=c.id AND r.status IN('open','counted')) FOR UPDATE`,
          [s.companyId, id],
        )
      ).rows[0];
      ...
      const variance = lines.reduce(
        (n, l) =>
          n + Math.abs(Number(l.counted_quantity) - Number(l.system_quantity)),
        0,
      );
      const approval = await requestApproval(c, {
        companyId: s.companyId,
        userId: s.userId,
        operationType: "count_variance",
      ...
      for (const line of lines) {
        const current = Number(
          (
            await c.query(
              `SELECT coalesce(sum(quantity),0) AS q FROM inventory_ledger WHERE company_id=$1 AND item_id=$2 AND location_id=$3 AND lot_number IS NOT DISTINCT FROM $4 AND expiry_date IS NOT DISTINCT FROM $5::date`,
              [
                s.companyId,
                line.item_id,
                line.location_id,
        ...
        const delta = Number(line.counted_quantity) - current;
        if (delta !== 0)
          await c.query(
    ...
    return Response.redirect(
      new URL(
        pending ? "/app/approvals?requested=1" : `/app/counts/${id}?approved=1`,
        request.url,
      ),
      303,
    );
    ...
    const code =
      e instanceof Error && e.message === "STALE" ? "stale" : "approve";
    return Response.redirect(
      new URL(`/app/counts/${id}?error=${code}`, request.url),
      303,
    );
  }
}
