...
export async function POST(request:Request){
  ...
  const appHost=new URL(process.env.APP_URL||'https://invalid.local').hostname;
  const throttleDisabled=process.env.LOCAL_AUTH_THROTTLE_DISABLED==='true'&&['localhost','127.0.0.1'].includes(appHost);
  if(!throttleDisabled){const throttle=(await db().query<{blocked:boolean}>(`SELECT blocked_until>now() AS blocked FROM login_throttles WHERE email=$1`,[parsed.data.email])).rows[0];if(throttle?.blocked)return Response.redirect(new URL('/signin?error=locked',request.url),303)}
  ...
  const storeProfile=user.role==='viewer'&&await withTenant(user.company_id,async c=>Boolean((await c.query('SELECT 1 FROM store_user_assignments WHERE company_id=$1 AND user_id=$2',[user.company_id,user.id])).rowCount));
