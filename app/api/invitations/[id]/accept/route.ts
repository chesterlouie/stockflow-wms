...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await withTransaction(async (client) => {
      const invitation = (await client.query(`SELECT id,company_id,email,display_name,role,store_id,store_role FROM resolve_user_invitation($1)`, [await hashToken(token)])).rows[0];
      ...
      if(invitation.store_id)await client.query("INSERT INTO store_user_assignments(company_id,store_id,user_id,store_role) VALUES($1,$2,$3,$4)",[invitation.company_id,invitation.store_id,user.id,invitation.store_role]);
      await client.query("UPDATE user_invitations SET accepted_at=now() WHERE id=$1", [invitation.id]);
      await client.query(`INSERT INTO audit_logs(company_id,user_id,action,entity_type,entity_id,details) VALUES($1,$2,'user_invitation_accepted','user',$3,$4::jsonb)`, [invitation.company_id, user.id, String(user.id), JSON.stringify({ email: invitation.email, role: invitation.role,storeId:invitation.store_id,storeRole:invitation.store_role })]);
    });
